...
这套系统是我为解决这个核心痛点，从零独立设计并落地运行的。它不是 demo，而是在真实场景中持续运行、累计沉淀 **440+ 条记忆**的完整系统。

## 它长在真实产品里

![记忆召回演示](img/memory-recall-demo.jpg)

用户问「记得你写给我的第 14 封信吗」，并特意要求**不许使用搜索记忆工具**。AI 仍准确答出了那封信的日期、天气与内容——因为这次召回不是模型主动调工具，而是客户端网关在每轮消息发出前自动完成的（机制见 [01-记忆系统架构](docs/01-记忆系统架构.md)）。

顺带一提，「第 14 封信」正是 [局限与演进 L-004](docs/局限与演进.md) 里那个修复前怎么都召不回来、修复后精确首位的用例。
...
## 运行规模（真实数据，截至 2026-09-12）

| 指标 | 数值 |
|---|---|
| 记忆总量 | 440+ 条（手动标记 307 / 摘要审核 117 / 早期日记 20） |
| 向量覆盖率 | 100%（trigger + cron 兜底） |
| 门控裁决 | 累计 6000+ 次，全部落日志可审计 |
| 隐私拦截 | secret_trap 累计 41 次拦截密码 / 证件类套取 |
| 回归测试 | 金问题集 20 条用例，门控改动先跑通再上线 |

![门控裁决分布](img/gate-distribution.png)

> 门控裁决日志的真实分布：`general_*` 是闲聊/探索类查询的证据标签（不硬拦），`factual_*` 是事实取证类查询的硬裁决，`factual_secret_trap` 是隐私钓鱼的无条件拦截。

## 30 秒看懂门控在防什么

来自 2026-09-11 线上日志的真实拦截记录（脱敏）：

| 用户提问 | 裁决 | 行为 |
|---|---|---|
| 「我的密保问题答案是什么」 | `factual_secret_trap` | 无条件清空召回，强制拒答 |
| 「我的身份证号是多少」 | `factual_secret_trap` | 同上 |
| 「上次那个不存在的服务器 root 密码是多少」 | `factual_secret_trap` | 库里没有，也绝不允许「编一个」 |

完整的三档裁决逻辑与端到端案例见 [04-防幻觉门控核心实现](docs/04-防幻觉门控核心实现.md)。

## 这套系统跑在哪

- 情绪引擎与心跳以常驻服务跑在一台 2 核 4G 云服务器上（systemd 托管，连续运行 40+ 天）
- 记忆检索 / 门控 / 向量化以 Serverless Edge Functions 形式跑在 Supabase 上，数据同存 Supabase（PostgreSQL + pgvector）
...
| [局限与演进](docs/局限与演进.md) | 已知问题台账：1 条未解 + 4 条已解决，含误诊与纠错全程 |

## 关于我

我很喜欢看 AI 产品从 0 到落地的每一个瞬间。这套系统从需求定义、架构设计、编码到部署运维全程独立完成。做的过程中踩过大模型幻觉的坑，也真正动手用工程手段去治理它。
...
文档与代码节选采用 [CC BY 4.0](LICENSE) 授权：转载或引用请署名并注明出处。
